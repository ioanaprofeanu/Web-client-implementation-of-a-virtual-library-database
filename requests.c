                            ...
                            char *cookie, int cookies_count, char *jwt_token)
{
    ...
    // Step 1: write the method name, URL, request params
    // (if any) and protocol type
    ...
    // Step 3 (optional): add headers, jwt and/or cookies, according
    // to the protocol format
    if (cookie != NULL && cookies_count == 1) {
                            ...
                            char *cookie, int cookies_count, char *jwt_token)
{
    ...
    // Step 3: add necessary headers (Content-Type and
    // Content-Length are mandatory)
    ...
    // Step 4 (optional): add cookies and jwt
    if (cookie != NULL && cookies_count == 1) {
        sprintf(line, "Cookie: %s", cookie);
        compute_message(message, line);
    }
